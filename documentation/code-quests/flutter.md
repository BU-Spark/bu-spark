...
- **Routing** — The system that controls navigation between screens (pages) in an app. In Flutter, routing determines which widget/screen is shown when a user taps a button, link, or performs an action (for example moving from a home screen to a settings page).  
...
- **Emulator** — A virtual mobile device that runs on your computer and simulates a real phone or tablet, allowing you to install, run, and test your app without needing a physical device.


## SPECIFIC COMMANDS/CODE SNIPPETS

- `setState()` — Notifies Flutter that a widget’s data has changed and the UI should rebuild to reflect the update.

- `flutter create [projectname]` — Generates a new Flutter project with the default folder structure and starter files.

- `flutter run` — Builds the app and launches it on a connected device or emulator.

- `r` — Hot reload shortcut in the terminal that refreshes the app UI without restarting the entire application.

- `Widget build(BuildContext context)` — The required method that describes and returns the UI layout for a widget.

- `createState()` — Connects a StatefulWidget to its State object where its mutable data and logic are stored.

- `flutter doctor` — Checks whether the Flutter SDK and required development tools are correctly installed and configured.

- `@override` — An annotation that indicates a method is replacing a method inherited from a parent class.
...
Open a new terminal in VS Code and navigate to a file or directory to hold your Flutter projects (using `cd [address]`). Then, you can type `flutter create [projectname]`
